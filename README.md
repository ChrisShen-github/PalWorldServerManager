...
- 读取官方 REST API：服务器版本、FPS、在线训练家、世界天数与基地数量；训练家详情可查看 Steam 用户 ID、脱敏 IP 和坐标，并在本地地图定位。
- 游戏内管理：发送全服公告、保存世界、踢出、封禁与解除封禁在线训练家。
...
在侧边栏打开 **游戏内管理**。此页使用 Palworld 官方 REST API，提供以下实时操作：

- 向所有在线训练家发送公告。
- 立即请求游戏保存当前世界（不会创建面板备份，也不会停服）。
- 对在线训练家执行踢出或封禁；执行前会再次确认。
- 根据 Steam 用户 ID 解除封禁。

需要先在 **世界规则与安装** 中保存管理员密码，并重启游戏服务器使 REST API 生效。管理员密码只由面板后端用于请求 API，不会返回给浏览器。官方 API 目前没有封禁列表查询端点，因此解除封禁需要手动输入 Steam 用户 ID。
