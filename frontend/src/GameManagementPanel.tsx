...
type Player = { name: string; account_name: string; player_id: string; ping: number; level: number; building_count: number };
type Overview = { status: "online" | "offline" | "demo"; message: string; server: { name: string; version: string }; metrics: { current_players: number; max_players: number; server_fps: number }; players: Player[] };
type Pending = { kind: "kick" | "ban"; player: Player } | { kind: "unban"; userId: string };
...
export default function GameManagementPanel() {
  ...
  return <div className="game-page">
    ...
    <section className="game-card game-players" aria-labelledby="game-players-title"><header><div><p className="eyebrow">LIVE TRAINERS</p><h2 id="game-players-title">在线训练家</h2><p>踢出与封禁会要求再次确认，消息会显示给对应训练家。</p></div><strong>{overview.players.length} 人在线</strong></header>{overview.players.length ? <div className="game-player-list">{overview.players.map((player) => <article key={player.player_id}><div className="game-player-avatar">{player.name.slice(0, 1)}</div><div className="game-player-info"><strong>{player.name}</strong><small>@{player.account_name} · {player.player_id}</small></div><div className="game-player-meta"><span>Lv. {player.level}</span><span>{player.ping.toFixed(0)} ms</span><span>{player.building_count} 建造物</span></div><div className="game-player-actions"><button className="button button-secondary" disabled={!available || busy !== null} onClick={() => { setReason("请遵守服务器规则。"); setPending({ kind: "kick", player }); }} type="button">踢出</button><button className="button button-danger" disabled={!available || busy !== null} onClick={() => { setReason("请遵守服务器规则。"); setPending({ kind: "ban", player }); }} type="button">封禁</button></div></article>)}</div> : <div className="game-empty"><PalIcon name="trainers" /><strong>{loading ? "正在读取在线训练家…" : "当前没有训练家在线"}</strong><span>{overview.message}</span></div>}</section>
